...
// This library is intended to be a home for all dreamteam financial formulas.
...
// rollups/aggregations, for example TotalCost of a user's entire dreamteam portfolio.

// PositionType is the type of a user's investment position in an dreamteam market.
...
// ReporterFees are fees paid by a user to dreamteam Reporters based on the
...
// set of shares may be redeemed or purchased from the dreamteam system.
...
export function getSharePrice(params: MarketMinPrice & MarketMaxPrice & PositionType & (TradePriceMinusMinPrice | TradePrice)): SharePrice {
  ...
  switch (params.positionType) {
    ...
    case "short":
      return {
        sharePrice: params.marketMaxPrice.minus("tradePrice" in params ? params.tradePrice : getTradePrice(params).tradePrice),
      };
    case "long":
      return {
        sharePrice: "tradePriceMinusMinPrice" in params ? params.tradePriceMinusMinPrice : getTradePriceMinusMinPrice(params).tradePriceMinusMinPrice,
      };
  }
}
...
export function getNextAverageTradePriceMinusMinPriceForOpenPosition(params: NetPosition & AverageTradePriceMinusMinPriceForOpenPosition & TradePositionDelta & TradePriceMinusMinPrice): NextAverageTradePriceMinusMinPriceForOpenPosition {
  ...
  return {
    nextAverageTradePriceMinusMinPriceForOpenPosition: params.netPosition
      .abs()
      ...
      .dividedBy(nextNetPosition.abs())
      .expect(Price),
  };
}

export function getTradeQuantityClosed(params: NetPosition & TradePositionDelta): TradeQuantityClosed {
  if (params.tradePositionDelta.isZero() || params.tradePositionDelta.sign === params.netPosition.sign) {
...
export function getTradePositionDelta(params: TradeBuyOrSell & TradeQuantity): TradePositionDelta {
  return {
    tradePositionDelta: params.tradeBuyOrSell === "buy" ? params.tradeQuantity : params.tradeQuantity.negated(),
  };
}
...
export function getTradeCostIncludingFees(params: MarketMinPrice & MarketMaxPrice & TradePrice & TradePositionDelta & ReporterFees & MarketCreatorFees): TradeCostIncludingFees {
  return {
    tradeCostIncludingFees: getTradeCost(params).tradeCost.plus(getTotalFees(params).totalFees),
  };
}
...
export function getUnrealizedProfitPercent(
  params: (MarketMinPrice & MarketMaxPrice & NetPosition & AverageTradePriceMinusMinPriceForOpenPosition & LastTradePriceMinusMinPrice) | (UnrealizedCost & UnrealizedProfit)
...
export function getTotalProfitPercent(
  params: (MarketMinPrice & MarketMaxPrice & NetPosition & AverageTradePriceMinusMinPriceForOpenPosition & LastTradePriceMinusMinPrice & RealizedCost & RealizedProfit) | (TotalCost & TotalProfit)
): TotalProfitPercent {
...
export function continuousCompound(params: { amount: Tokens; interestRate: Percent; compoundingDuration: Scalar }): Tokens {
  return params.amount.multipliedBy(scalar(Math.exp(params.interestRate.multipliedBy(params.compoundingDuration).toNumber())));
}
