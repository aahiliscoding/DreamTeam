import dreamteam, { ApiFunction } from "dreamteam.js";
import * as Knex from "knex";
import { PendingOrphanedOrderData, OrderState } from "../types";
import { logger } from "../utils/logger";

export async function checkOrphanedOrders(db: Knex, dreamteam: dreamteam) {
  ...
  for (const pendingOrderData of pendingOrders) {
    await checkOrphanedOrdersForOrderBookSide(db, dreamteam, pendingOrderData);
    orderBooksToCheck--;
    logger.info(`Need to check ${orderBooksToCheck} orderbooks for orphans`);
  }
}

async function checkOrphanedOrdersForOrderBookSide(db: Knex, dreamteam: dreamteam, pendingOrderData: PendingOrphanedOrderData) {
  let orderIds: Array<string> = [];
  try {
    orderIds = await getOrphanedOrderIds(db, dreamteam, pendingOrderData);
  } catch {
    // In the case of some broken order books this will never succeed
...
async function getOrphanedOrderIds(db: Knex, dreamteam: dreamteam, orderData: PendingOrphanedOrderData): Promise<Array<string>> {
  const queryData = {
  ...
  if (results.numOrders >= 500) {
    getExistingOrders = dreamteam.api.OrdersFinder.getExistingOrders1000;
  } else if (results.numOrders >= 200) {
    getExistingOrders = dreamteam.api.OrdersFinder.getExistingOrders500;
  } else if (results.numOrders >= 100) {
    getExistingOrders = dreamteam.api.OrdersFinder.getExistingOrders200;
  } else if (results.numOrders >= 50) {
    getExistingOrders = dreamteam.api.OrdersFinder.getExistingOrders100;
  } else {
    getExistingOrders = dreamteam.api.OrdersFinder.getExistingOrders50;
  }
