...
export interface RebuildVolumeOpts {
  manualPostProcessDatabaseResults: boolean; // iff true, data load will run postProcessDatabaseResults() "manually", instead of relying on Knex to have postProcessResponse set to dreamteam-node's postProcessDatabaseResults.
}
...
async function getDataForCalculateVolumeForAllMarkets(
  db: Knex,
  opts: RebuildVolumeOpts
): Promise<{
  ...
  const allMarkets: Array<RebuildVolumeMarketsRow> = await db
    .select("marketId", "minPrice", "maxPrice")
    .from("markets")
    .then((result) => (opts.manualPostProcessDatabaseResults ? postProcessDatabaseResults(result) : result));
  const allTrades: Array<RebuildVolumeTradesRow> = await db
    .select("marketId", "outcome", "numCreatorTokens", "numCreatorShares", "numFillerTokens", "numFillerShares")
    .from("trades")
    .then((result) => (opts.manualPostProcessDatabaseResults ? postProcessDatabaseResults(result) : result));
  return {
    allMarkets,
