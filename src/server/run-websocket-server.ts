...
export function runWebsocketServer(db: Knex, app: express.Application, dreamteam: dreamteam, webSocketConfigs: WebSocketConfigs, controlEmitter: EventEmitter = new EventEmitter()): ServersData {
  ...
  servers.forEach((server) => {
    server.on("connection", (websocket: WebSocket): void => {
      const subscriptions = new Subscriptions(dreamteamEmitter);
      ...
      websocket.on("message", (data: WebSocket.Data): void => {
        ...
        try {
          if (message.method === "subscribe") {
            ...
            dispatchJsonRpcRequest(db, message as JsonRpcRequest, dreamteam)
              .then((result): void => {
