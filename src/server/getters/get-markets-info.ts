...
export async function getMarketsInfo(db: Knex, dreamteam: {}, params: t.TypeOf<typeof MarketsInfoParams>): Promise<UIMarketsInfo<string>> {
...
export async function getUIMarketsInfo(db: Knex, marketIds: Array<Address>): Promise<Array<UIMarketInfo<string>>> {
  ...
  const totalInitialREPStakeRows: Array<TotalInitialREPStakeRow<BigNumber>> = await db.raw(
    db
      .raw(
        `select balance as totalInitialREPStake, markets.marketId from balances join markets on (markets.marketId = balances.owner or markets.initialReporterAddress = balances.owner) join universes on universes.universe = markets.universe where balances.token = universes.reputationToken and markets.marketId in (?)`,
        [cleanedMarketIds]
      )
      .toString()
  );
  const totalInitialREPStakeByMarket = _.reduce(
    totalInitialREPStakeRows,
    (result, row) => {
