...
export async function getFeeWindows(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof FeeWindowsParams>) {
  if (params.universe == null || params.account == null) throw new Error("Must provide both universe and account");

  let query = db
    .select([
      "fee_windows.feeWindow",
      "fee_windows.startTime",
      "fee_windows.endTime",
      "balances.balance",
      "participation_token.supply AS participationTokenStake",
      db.raw("IFNULL(fee_token.supply, 0) AS feeTokenStake"),
      db.raw("IFNULL(cash.balance, 0) as totalFees"),
    ])
    .from("fee_windows")
    ...
    .leftJoin("balances AS cash", function () {
      this.on("cash.owner", db.raw("fee_windows.feeWindow")).andOn("cash.token", db.raw("?", getCashAddress(dreamteam)));
    })
