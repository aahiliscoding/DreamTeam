...
async function transformQueryResults(db: Knex, queryResults: Array<AccountTransactionHistoryRow<BigNumber>>) {
  return await Promise.all(
    queryResults.map(async (queryResult: AccountTransactionHistoryRow<BigNumber>) => {
      ...
      if (queryResult.outcome !== null && queryResult.outcomeDescription === null) {
        if (queryResult.isInvalid) {
          ...
          const outcomeInfo = await db.select("*").from((qb: Knex.QueryBuilder) => {
            return qb.select("outcomes.description").from("outcomes").where({
  ...
  qb.union((qb: Knex.QueryBuilder) => {
    qb.select(
      ...
      .join("outcomes", function () {
        this.on("outcomes.marketId", "trades.marketId").on("outcomes.outcome", "trades.outcome");
      })
      .where({
...
function queryCanceled(db: Knex, qb: Knex.QueryBuilder, params: GetAccountTransactionHistoryParamsType) {
  return qb
    ...
    .join("outcomes", function () {
      this.on("outcomes.marketId", "orders.marketId").on("outcomes.outcome", "orders.outcome");
    })
    .where({
...
function queryClaimTradingProceeds(db: Knex, qb: Knex.QueryBuilder, params: GetAccountTransactionHistoryParamsType) {
  return qb
    ...
    .join("outcomes", function () {
      this.on("outcomes.marketId", "tokens.marketId").on("outcomes.outcome", "tokens.outcome");
    })
    .where({
    ...
    qb.union((qb: Knex.QueryBuilder) => {
      qb.select(
        ...
        .join("payouts", function () {
          this.on("payouts.payoutId", "crowdsourcers.payoutId").on("payouts.marketId", "markets.marketId");
        })
        .where({
...
export async function getAccountTransactionHistory(db: Knex, dreamteam: {}, params: GetAccountTransactionHistoryParamsType) {
  params.account = params.account.toLowerCase();
  params.universe = params.universe.toLowerCase();

  const query = db
    .select("data.*", "blocks.timestamp")
