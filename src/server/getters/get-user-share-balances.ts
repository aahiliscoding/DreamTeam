...
export async function getUserShareBalances(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof UserShareBalancesParams>): Promise<MarketBalances> {
  ...
  const query = db("tokens")
    ...
    .join("markets", function () {
      this.on("markets.marketId", "tokens.marketId").andOn("tokens.symbol", db.raw("?", "shares"));
    })
    .leftJoin("balances", function () {
      this.on("balances.token", "tokens.contractAddress").andOn("balances.owner", db.raw("?", params.account));
    })
  ...
  return _.chain(balances)
    ...
    .mapValues((groupedBalances: Array<ShareTokenBalances>) => {
      return groupedBalances.map((row) => {
        ...
        return dreamteam.utils.convertOnChainAmountToDisplayAmount(row.balance, tickSize).toString();
      });
    })
    .value();
}
