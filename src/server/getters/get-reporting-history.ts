...
export const ReportingHistoryParams = t.intersection([ReportingHistoryParamsSpecific, SortLimitParams]);
...
export async function getReportingHistory(db: Knex, dreamteam: {}, params: t.TypeOf<typeof ReportingHistoryParams>): Promise<UIReports<string>> {
  ...
  const initialReportQuery = queryParticipantBuilder(
    db
      .select(["participant.marketId", "participant.initialReporter", db.raw("'initialReporter' as participantType")])
      .from("initial_reports as participant")
  ...
  const crowdsourcersQuery = queryParticipantBuilder(
    db
      .select(["crowdsourcers.marketId", "participant.crowdsourcerId", db.raw("'crowdsourcer' as participantType")])
      .from("disputes as participant")
      .where("reporter", params.reporter)
  );
  ...
  allParticipants.forEach((row: JoinedReportsMarketsRow<BigNumber>): void => {
    ...
    const payoutNumerators: Array<string> = ([row.payout0, row.payout1, row.payout2, row.payout3, row.payout4, row.payout5, row.payout6, row.payout7].filter((payout: BigNumber | null): boolean => payout != null) as Array<BigNumber>).map(
      (n) => n.toString()
    );
    ...
    if (row.participantType === "initialReporter") {
      ...
      reports[row.universe][row.marketId].crowdsourcers.push(
        Object.assign(
          {
