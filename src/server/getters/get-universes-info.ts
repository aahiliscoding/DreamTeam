...
export async function getUniversesInfo(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof UniverseInfoParams>): Promise<Array<UIUniverseInfoRow<string>>> {
  ...
  const query = db
    .select([
      ...
      "balances.balance",
      "token_supply.supply",
      db.raw("count(markets.marketId) as numMarkets"),
    ])
    .from("universes")
    ...
    .leftJoin("balances", function () {
      this.on("balances.owner", db.raw("?", [params.account])).on("balances.token", "universes.reputationToken");
    })
