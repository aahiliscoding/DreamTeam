...
export async function getProceedTradeRows(db: Knex, dreamteam: dreamteam, marketIds: Array<Address>, account: Address, endTime: number | null): Promise<Array<ProceedTradesRow<BigNumber>>> {
  ...
  marketsQuery.join("trading_proceeds", function () {
    this.on("trading_proceeds.shareToken", "shareTokens.contractAddress");
  });
  marketsQuery.join("tokens AS shareTokens", function () {
    this.on("shareTokens.marketId", "markets.marketId").andOn("symbol", db.raw("?", "shares"));
  });
  marketsQuery.join("balances", function () {
    this.on("balances.token", "shareTokens.contractAddress").andOn("balances.owner", db.raw("?", account));
  });
  ...
  return _.map(winningPayoutRows, (row: WinningPayoutRow): ProceedTradesRow<BigNumber> => {
    ...
    const amount = dreamteam.utils.convertOnChainAmountToDisplayAmount(row.balance, tickSize);
