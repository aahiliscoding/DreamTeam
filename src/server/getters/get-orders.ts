...
export const OrdersParams = t.intersection([OrdersParamsSpecific, SortLimitParams]);
...
export async function getOrders(db: Knex, dreamteam: {}, params: t.TypeOf<typeof OrdersParams>): Promise<UIOrders<string>> {
  ...
  const query: Knex.QueryBuilder = db
    .select(["orders.*", "blocks.blockHash", "blocks.timestamp as creationTime", "orders_canceled.transactionHash as canceledTransactionHash", "orders_canceled.blockNumber as canceledBlockNumber", "canceledBlock.timestamp as canceledTime"])
    .from("orders");
