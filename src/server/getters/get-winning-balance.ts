...
export async function getWinningBalance(db: Knex, dreamteam: {}, params: t.TypeOf<typeof WinningBalanceParams>): Promise<Array<MarketWinnings>> {
  ...
  marketsQuery.join("balances", function () {
    this.on("balances.token", "shareTokens.contractAddress").andOn("balances.owner", db.raw("?", params.account));
  });
