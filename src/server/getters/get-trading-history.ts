...
export const TradingHistoryParamsSpecific = t.type({
  ...
  marketId: t.union([t.string, t.null, t.undefined, t.array(t.string)]), // eg. marketId: ["0x123", "0x456"] will restrict trading history to only those two markets; filtering by list of marketIds was added later and we did it this way to maintain backwards compatibility
...
export const TradingHistoryParams = t.intersection([TradingHistoryParamsSpecific, SortLimitParams]);

// Look up a user or market trading history. Must provide universe OR market. Outcome and orderType are optional parameters.
export async function getTradingHistory(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof TradingHistoryParams>): Promise<Array<UITrade>> {
  ...
  return userTradingHistory.map((trade: TradingHistoryRow): UITrade => {
    ...
    return Object.assign(_.pick(trade, ["transactionHash", "logIndex", "orderId", "filler", "creator", "marketId", "outcome", "shareToken", "timestamp", "tradeGroupId"]), {
