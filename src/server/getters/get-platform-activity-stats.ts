...
async function getVolume(db: Knex, startBlock: number, endBlock: number, params: PlatformActivityStatsParamsType): Promise<Knex.QueryBuilder> {
  return db.select("amount as volume").from("trades").innerJoin("markets", "markets.marketId", "trades.marketId").whereBetween("trades.blockNumber", [startBlock, endBlock]).andWhere("markets.universe", params.universe);
}
...
async function getAmountStaked(db: Knex, startBlock: number, endBlock: number, params: PlatformActivityStatsParamsType): Promise<Knex.QueryBuilder> {
  return db
    ...
    .whereIn("crowdsourcers.marketId", function (this: Knex.QueryBuilder) {
      this.select("markets.marketId").from("markets").where("universe", params.universe);
    });
}
...
async function getActiveUsers(db: Knex, startBlock: number, endBlock: number, params: PlatformActivityStatsParamsType): Promise<Knex.QueryBuilder> {
  return db.countDistinct("account as activeUsers").from(function (this: Knex.QueryBuilder) {
    this.select("filler as account")
      .from("trades")
...
async function getNumberOfTrades(db: Knex, startBlock: number, endBlock: number, params: PlatformActivityStatsParamsType): Promise<Knex.QueryBuilder> {
  return db
    ...
    .whereBetween("trades.blockNumber", [startBlock, endBlock])
    .whereIn("trades.marketId", function (this: Knex.QueryBuilder) {
      this.select("markets.marketId").from("markets").where("markets.universe", params.universe);
    });
}

export async function getPlatformActivityStats(db: Knex, dreamteam: dreamteam, params: PlatformActivityStatsParamsType): Promise<PlatformActivityResult> {
  // guards
  if (params.startTime && params.endTime && params.startTime > params.endTime) throw new Error("startTime must be less than or equal to endTime");
  ...
  const sql = db
    ...
    .leftJoin(db.raw("markets on universe = ? and markets.creationBlockNumber between startBlock and endBlock", [params.universe]));
  ...
  if (!startBlock || !endBlock || startBlock > endBlock) throw new Error("startTime/endTime error");
  ...
  const openInterestPromise = await dreamteam.api.Universe.getOpenInterestInAttoEth({});
