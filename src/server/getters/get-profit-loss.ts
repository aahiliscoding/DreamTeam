...
export interface ProfitLossResult
  extends Timestamped, // profit and loss as of this timestamp
    FrozenFunds {
  // funds the user froze to be in this position (see FrozenFunds docs)
...
export const GetProfitLossSummaryParams = t.intersection([
  t.type({
    universe: t.string,
    account: t.string,
    marketId: t.union([t.string, t.null]),
...
function getProfitResultsForTimestamp(
  ...
  outcomeValuesAtTimestamp: Array<OutcomeValueTimeseries> | null,
  ...
  const unsortedResults: Array<ProfitLossResult> = plsAtTimestamp.map((outcomePl) => {
    ...
    const lastTradePriceMinusMinPrice24hAgo: Price =
      lastTradePriceMinusMinPrice24hAgoByOutcomeByMarketId[outcomePl.marketId] !== undefined
        ? lastTradePriceMinusMinPrice24hAgoByOutcomeByMarketId[outcomePl.marketId][outcome] !== undefined
          ? new Price(lastTradePriceMinusMinPrice24hAgoByOutcomeByMarketId[outcomePl.marketId][outcome].value)
          : Price.ZERO
        : Price.ZERO;
    ...
    const oldestTradePriceMinusMinPriceUserPaidForOpenPositionInLast24h: Price | undefined =
      oldestTradePriceMinusMinPriceUserPaidForOpenPositionInLast24hByOutcomeByMarketId[outcomePl.marketId] !== undefined
...
function getProfitResultsForMarket(
  marketPls: Array<Array<ProfitLossTimeseries>>,
  marketOutcomeValues: Array<Array<OutcomeValueTimeseries>>,
  ...
  return _.map(marketPls, (outcomePLsAtTimestamp, timestampIndex) => {
    ...
    const sortedOutcomeValues = _.reduce(
      _.range(numOutcomes),
      (result, outcomeIndex) => {
...
async function getProfitLossData(db: Knex, params: GetProfitLossParamsType): Promise<ProfitLossData> {
  ...
  const profits: Dictionary<Dictionary<Array<ProfitLossTimeseries>>> = _.reduce(
    marketProfits,
    (result, value, key) => {
  ...
  const outcomeValues: Dictionary<Dictionary<Array<OutcomeValueTimeseries>>> = _.reduce(
    marketOutcomeValues,
    (result, value, key) => {
      result[key] = _.groupBy(value, (r) => r.outcome);
      return result;
...
export async function getAllOutcomesProfitLoss(db: Knex, params: GetProfitLossParamsType): Promise<AllOutcomesProfitLoss> {
  ...
  const marketOutcomes = _.fromPairs(
    _.values(
      _.mapValues(profits, (pls) => {
...
export async function getProfitLoss(db: Knex, dreamteam: dreamteam, params: GetProfitLossParamsType): Promise<Array<ProfitLossResult>> {
...
export async function getProfitLossSummary(db: Knex, dreamteam: dreamteam, params: GetProfitLossSummaryParamsType): Promise<NumericDictionary<ProfitLossResult>> {
  ...
  for (const days of [1, 30]) {
    ...
    const [startProfit, endProfit, ...rest] = await getProfitLoss(db, dreamteam, {
...
async function getLastTradePriceMinusMinPrice24hAgoByOutcomeByMarketId(db: Knex, now: number, params: GetProfitLossParamsType): Promise<Dictionary<Dictionary<OutcomeValueTimeseries>>> {
  ...
  const lastTradePriceMinusMinPrice24hAgoByOutcomeByMarketId: Dictionary<Dictionary<OutcomeValueTimeseries>> = _.reduce(
    lastTradePriceMinusMinPrice24hAgoByMarketId,
    (result, allPricesOneMarket, marketId) => {
...
async function getOldestTradePriceMinusMinPriceUserPaidForOpenPositionInLast24hByOutcomeByMarketId(db: Knex, now: number, params: GetProfitLossParamsType): Promise<Dictionary<Dictionary<Price>>> {
  ...
  const trades: Array<Pick<TradesRow<BigNumber>, "marketId" | "outcome" | "price"> & Pick<MarketsRow<BigNumber>, "minPrice">> = await db.raw(
    `
  ...
  return _.reduce(
    tradesByMarketId,
    (result, allTradesOneMarket, marketId) => {
      ...
      const priceByOutcome = _.mapValues(allTradesOneMarketByOutcome, (allTradesForOneOutcome) => {
        if (allTradesForOneOutcome.length !== 1) {
          throw new Error(
            `getOldestTradePriceMinusMinPriceUserPaidForOpenPositionInLast24hByOutcomeByMarketId: expected allTradesForOneOutcome to have lenght 1 because we select exactly one trade row per matching (marketId, outcome), allTradesForOneOutcome.length=${allTradesForOneOutcome.length}, params=${params}`
          );
        }
