...
export interface AggregatedTradingPosition
  extends Pick<
      ProfitLossResult,
      "realized" | "unrealized" | "total" | "unrealizedCost" | "realizedCost" | "totalCost" | "realizedPercent" | "unrealizedPercent" | "totalPercent" | "unrealizedRevenue" | "unrealizedRevenue24hAgo" | "unrealizedRevenue24hChangePercent"
    >,
    FrozenFunds {}
...
export interface GetUserTradingPositionsResponse {
  ...
  tradingPositionsPerMarket: {
    // per-market aggregation of trading positions
...
export async function getUserTradingPositions(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof UserTradingPositionsParams>): Promise<GetUserTradingPositionsResponse> {
  ...
  const rawPositionsMapping: { [key: string]: RawPosition } = _.reduce(
    rawPositions,
    (result, rawPosition) => {
      ...
      rawPosition.balance = dreamteam.utils.convertOnChainAmountToDisplayAmount(new BigNumber(rawPosition.balance, 10), tickSize);
  ...
  let positions: Array<TradingPosition> = _.flatten(
    _.map(profitsPerMarket, (outcomePls: Array<Array<ProfitLossResult>>) => {
      ...
      return _.map(lastTimestampPls, (plr) => {
        ...
        return Object.assign({ position }, plr);
      });
    })
  );
  ...
  const frozenFundsTotal: FrozenFunds | undefined = params.marketId
    ? undefined
    : {
...
async function getEthEscrowedInValidityBonds(db: Knex, marketCreator: Address): Promise<BigNumber> {
  const marketsRow: Array<Pick<MarketsRow<BigNumber>, "validityBondSize">> = await db
    .select("validityBondSize", "reportingState")
    .from("markets")
  ...
  for (const market of marketsRow) {
    ...
    totalValidityBonds = totalValidityBonds.plus(fixedPointToDecimal(market.validityBondSize, BN_WEI_PER_ETHER));
  }
