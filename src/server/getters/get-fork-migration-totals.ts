...
export async function getForkMigrationTotals(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof ForkMigrationTotalsParams>) {
  const query = db
    .select(["universe", "payouts.isInvalid", "payouts.payout0", "payouts.payout1", "payouts.payout2", "payouts.payout3", "payouts.payout4", "payouts.payout5", "payouts.payout6", "payouts.payout7", "token_supply.supply AS repTotal"])
    .from("universes")
  ...
  return forkMigrationTotals.reduce((acc: UIForkMigrationTotals<string>, cur) => {
    const payout: Array<string> = [cur.payout0, cur.payout1, cur.payout2, cur.payout3, cur.payout4, cur.payout5, cur.payout6, cur.payout7]
      .filter((payout: BigNumber | null): boolean => payout != null)
      .map((payout: BigNumber) => payout.toString());
