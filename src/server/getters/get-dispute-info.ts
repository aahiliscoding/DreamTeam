...
async function getCurrentStakes(db: Knex, marketIds: Array<Address>) {
  const results: Array<ActiveCrowdsourcer> = await db("crowdsourcers").select(["marketId", "payoutId", "amountStaked", "size"]).whereNull("completed").whereIn("crowdsourcers.marketId", marketIds);
...
async function getCompletedStakes(db: Knex, marketIds: Array<Address>) {
  const results: Array<StakeRow> = await db.select("marketId", "payoutId", "amountStaked").from((builder: QueryBuilder) => {
    return builder
      .from("crowdsourcers")
      ...
      .union((builder: QueryBuilder) => {
        return builder.from("initial_reports").select("marketId", "payoutId", "amountStaked").whereIn("marketId", marketIds);
      });
  });
...
export async function getDisputeInfo(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof DisputeInfoParams>): Promise<Array<UIStakeInfo<string> | null>> {
  ...
  const disputeDetailsByMarket = _.map(cleanMarketIds, (marketId: Address): DisputesResult => {
    return {
...
function reshapeStakeRowToUIStakeInfo(stakeRows: DisputesResult): UIStakeInfo<string> | null {
  ...
  const totalCompletedStakeOnAllPayouts = _.reduce(stakeRows.stakesCompleted, (result: BigNumber, completedStake: StakeRow): BigNumber => result.plus(completedStake.amountStaked), ZERO);
  ...
  const stakeResults = _.map(stakeRows.payouts, (payout: PayoutRow<BigNumber>): StakeDetails<BigNumber> => {
    ...
    return Object.assign({}, normalizePayouts(payout), currentAmounts, {
