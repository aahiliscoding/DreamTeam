...
export async function getAccountTimeRangedStats(db: Knex, dreamteam: dreamteam, params: AccountTimeRangedStatsParamsType): Promise<AccountTimeRangeResult> {
  // guards
  ...
  const sql = db
    ...
    .leftJoin(db.raw("markets on universe=? and markets.marketCreator=? and markets.creationBlockNumber between startBlock and endBlock", [params.universe, params.account]))
    .leftJoin(
      db.raw("trades as trades on (creator=? or filler=?) and (trades.blockNumber between startBlock and endblock) AND trades.marketId in (select markets.marketId from markets where universe=?)", [
        params.account,
        params.account,
        params.universe,
      ])
    ...
    .leftJoin(
      db.raw("trading_proceeds as proceeds on account=? and (proceeds.blockNumber between startBlock and endBlock) and proceeds.marketId in (select markets.marketId from markets where universe=?)", [params.account, params.universe])
    );
