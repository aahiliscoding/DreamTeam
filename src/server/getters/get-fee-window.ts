...
async function fabricateFeeWindow(db: Knex, dreamteam: dreamteam, universe: Address, targetTime: number): Promise<UIFeeWindowCurrent<string> | null> {
  ...
  const feeWindowId = Math.floor(targetTime / dreamteam.constants.CONTRACT_INTERVAL.DISPUTE_ROUND_DURATION_SECONDS);
  const startTime = feeWindowId * dreamteam.constants.CONTRACT_INTERVAL.DISPUTE_ROUND_DURATION_SECONDS;
  const endTime = (feeWindowId + 1) * dreamteam.constants.CONTRACT_INTERVAL.DISPUTE_ROUND_DURATION_SECONDS;
  return {
    feeWindow: null,
    feeWindowId,
    startTime,
...
async function getFeeWindowEthFees(db: Knex, dreamteam: dreamteam, feeWindow: Address): Promise<BigNumber> {
  const feeWindowEthFeesQuery = db("balances").first("balance").where("owner", feeWindow).where("token", getCashAddress(dreamteam));
...
async function getFeeWindowRepStaked(db: Knex, feeWindow: Address, feeToken: Address) {
  const feeWindowRepStakedQuery = db("token_supply").select("token", "supply").whereIn("token_supply.token", [feeWindow, feeToken]);
...
async function getParticipantContributions(db: Knex, feeWindow: Address, reporter: Address) {
  const participantQuery = db.select("type", "reporterBalance as amountStaked").from("all_participants").where("feeWindow", feeWindow).where("reporter", reporter);
...
async function getParticipationTokens(db: Knex, feeWindow: Address, reporter: Address) {
  const participationTokenQuery = db
    .first(["participationToken.balance AS amountStaked"])
    .from("fee_windows")
...
export async function getFeeWindow(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof FeeWindowParams>): Promise<UIFeeWindowCurrent<string> | null> {
  if (params.feeWindowState == null && params.feeWindow == null) throw new Error("Must provide either a feeWindowState OR feeWindow");
  const query = db.select(["endTime", "feeWindow", "feeWindowId", "startTime", "universe", "feeToken"]).first().from("fee_windows").where("universe", params.universe);
  ...
  if (params.feeWindowState != null) {
    ...
    const feeWindowDuration = dreamteam.constants.CONTRACT_INTERVAL.DISPUTE_ROUND_DURATION_SECONDS;
  ...
  if (!feeWindowRow) {
    ...
    return fabricateFeeWindow(db, dreamteam, params.universe, targetTime);
  }
  const feeWindowEthFees = await getFeeWindowEthFees(db, dreamteam, feeWindowRow.feeWindow);
  ...
  const feeWindowResponse = Object.assign(
    ...
    formatBigNumberAsFixed(feeWindowRepStaked),
    feeWindowRow
  );
