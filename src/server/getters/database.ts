...
export async function queryModifierDB<T>(query: Knex.QueryBuilder, defaultSortBy: string, defaultSortOrder: string, sortLimitParams: Partial<SortLimit>): Promise<Array<T>> {
...
async function queryModifierUserland<T>(db: Knex, query: Knex.QueryBuilder, defaultSortBy: string, defaultSortOrder: string, sortLimitParams: Partial<SortLimit>): Promise<Array<T>> {
  ...
  if (sortLimitParams.limit == null && sortLimitParams.offset == null) return results;
  return results.slice(sortLimitParams.offset || 0, sortLimitParams.limit || results.length);
}

export async function queryModifier<T>(db: Knex, query: Knex.QueryBuilder, defaultSortBy: string, defaultSortOrder: string, sortLimitParams: Partial<SortLimit>): Promise<Array<T>> {
...
export function getMarketsWithReportingState(db: Knex, selectColumns?: Array<string>): Knex.QueryBuilder {
  ...
  return db.select(columns).from("markets").leftJoin("market_state", "markets.marketStateId", "market_state.marketStateId").leftJoin("blocks", "markets.creationBlockNumber", "blocks.blockNumber");
}
...
export function blockNumberForTimestampQuery(db: Knex, timestamp: number, prefix: string): Knex.QueryBuilder {
  ...
  if (timestamp === 0) {
    return db
      .select(`blockNumber as ${prefix}Block`, `timestamp as ${prefix}Time`)
      .from("blocks")
      .orderBy("blockNumber", prefix === "start" ? "asc" : "desc")
      .limit(1);
  }

  return db.select(`blockNumber as ${prefix}Block`, `timestamp as ${prefix}Time`).from("blocks").where("timestamp", ">=", timestamp).orderBy("blockNumber", "asc").limit(1);
}
...
export function getCashAddress(dreamteam: dreamteam) {
  return dreamteam.contracts.addresses[dreamteam.rpc.getNetworkID()].Cash;
}
