...
export async function getUnclaimedMarketCreatorFees(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof UnclaimedMarketCreatorFeesParams>): Promise<Array<UIMarketCreatorFee>> {
  ...
  marketsQuery.leftJoin("balances AS cash", function () {
    this.on("cash.owner", db.raw("markets.marketCreatorMailbox")).andOn("cash.token", db.raw("?", getCashAddress(dreamteam)));
  });
