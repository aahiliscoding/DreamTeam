...
export async function getCompleteSets(db: Knex, dreamteam: {}, params: t.TypeOf<typeof CompleteSetsParams>): Promise<CompleteSets> {
  const query = db.select("*").from("completeSets").where("account", params.account).where("universe", params.universe);
