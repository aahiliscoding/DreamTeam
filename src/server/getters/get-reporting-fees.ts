...
async function getUniverse(db: Knex, universe: Address): Promise<Address> {
  const query = db("fee_windows").first("universes.universe", "universes.parentUniverse").join("universes", "fee_windows.universe", "universes.universe").groupBy("fee_windows.universe").where("fee_windows.universe", universe);
...
function formatMarketInfo(initialReporters: Array<UnclaimedInitialReporterRow>, crowdsourcers: Array<UnclaimedCrowdsourcerRow>, forkedMarket: ForkedMarket, participantEthFees: Array<ParticipantEthFee>) {
  ...
  fees = _.reduce(
    ...
    (collection: any, crowdsourcer: UnclaimedCrowdsourcerRow) => {
      ...
      const fees = _.reduce(
        _.filter(participantEthFees, (ethFees) => ethFees.participantAddress === crowdsourcer.crowdsourcerId),
        (summation, fee) => summation.plus(fee.ethFees),
        ZERO
      );
  ...
  fees = _.reduce(
    ...
    (collection: any, row: UnclaimedInitialReporterRow) => {
      ...
      const fees = _.reduce(
        _.filter(participantEthFees, (ethFees) => ethFees.participantAddress === row.initialReporter),
        (summartion, fee) => summartion.plus(fee.ethFees),
        ZERO
      );
  ...
  for (i = 0; i < crowdsourcers.length; i++) {
    if (crowdsourcers[i].forking) {
      ...
      if (!keyedNonforkedMarkets[crowdsourcers[i].marketId]) {
        ...
        keyedNonforkedMarkets[crowdsourcers[i].marketId].unclaimedEthFees = _.reduce(
          [keyedNonforkedMarkets[crowdsourcers[i].marketId].initialReporter, crowdsourcers[i].crowdsourcerId],
          (aggregation, contractId: string) => aggregation.plus(fees[contractId] || ZERO),
          ZERO
...
async function getParticipationTokenEthFees(db: Knex, dreamteam: dreamteam, reporter: Address, universe: Address): Promise<Array<ParticipationTokenEthFee>> {
  ...
  participationTokenQuery.leftJoin("balances AS cashFeeWindow", function () {
    this.on("cashFeeWindow.owner", db.raw("fee_windows.feeWindow")).on("cashFeeWindow.token", db.raw("?", getCashAddress(dreamteam)));
  });
...
async function getParticipantEthFees(db: Knex, dreamteam: dreamteam, reporter: Address, universe: Address): Promise<Array<ParticipantEthFee>> {
  ...
  participantQuery.leftJoin("balances AS cashFeeWindow", function () {
    this.on("cashFeeWindow.owner", db.raw("feeToken.feeWindow")).on("cashFeeWindow.token", db.raw("?", getCashAddress(dreamteam)));
  });
  participantQuery.leftJoin("balances AS cashParticipant", function () {
    this.on("cashParticipant.owner", db.raw("participantAddress")).andOn("cashParticipant.token", db.raw("?", getCashAddress(dreamteam)));
  });
...
export async function getReportingFees(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof ReportingFeesParams>): Promise<FeeDetails> {
  const currentUniverse = await getUniverse(db, params.universe);
  const participantEthFees: Array<ParticipantEthFee> = await getParticipantEthFees(db, dreamteam, params.reporter, params.universe);
  const participationTokenEthFees: Array<ParticipationTokenEthFee> = await getParticipationTokenEthFees(db, dreamteam, params.reporter, params.universe);
