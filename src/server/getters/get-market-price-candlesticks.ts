...
export async function getMarketPriceCandlesticks(db: Knex, dreamteam: {}, params: t.TypeOf<typeof MarketPriceCandlesticksParams>): Promise<UICandlesticks> {
  const marketsRowPromise = db("markets").first("minPrice", "maxPrice").where({ marketId: params.marketId });
  const query = db
    .select(["trades.outcome", "trades.price", "trades.amount", "trades.numCreatorShares", "trades.numCreatorTokens", "trades.numFillerTokens", "trades.numFillerShares", "blocks.timestamp"])
    .from("trades")
    .join("blocks", "trades.blockNumber", "blocks.blockNumber")
  ...
  return _.mapValues(tradeRowsByOutcome, (outcomeTradeRows) => {
    ...
    return _.map(outcomeTradeRowsByPeriod, (trades: Array<MarketPriceHistoryRow>, startTimestamp): Candlestick => {
      ...
      const partialCandlestick: Pick<Candlestick, Exclude<keyof Candlestick, "tokenVolume">> = {
        // this Pick/Exclude stuff just allows us to set the Candlestick.tokenVolume later, but in a typesafe way that prevents typos.
        ...
        volume: _.reduce(
          trades,
          (totalVolume: BigNumber, tradeRow: MarketPriceHistoryRow) =>
            totalVolume.plus(
              volumeForTrade({
