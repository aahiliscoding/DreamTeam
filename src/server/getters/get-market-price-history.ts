...
export async function getMarketPriceHistory(db: Knex, dreamteam: dreamteam, params: t.TypeOf<typeof MarketPriceHistoryParams>): Promise<MarketPriceHistory<string>> {
  const tradesRows: Array<MarketPriceHistoryRow> = await db
    .select(["trades.outcome", "trades.price", "trades.amount", "blocks.timestamp"])
    .from("trades")
    .leftJoin("blocks", "trades.blockNumber", "blocks.blockNumber")
  ...
  tradesRows.forEach((trade: MarketPriceHistoryRow): void => {
    ...
    marketPriceHistory[trade.outcome].push(
      formatBigNumberAsFixed<TimestampedPriceAmount<BigNumber>, TimestampedPriceAmount<string>>({
        price: trade.price,
